...
def test_catalog_includes_media_and_public_authority_categories() -> None:
    ...
    assert publisher_profile("ftc.gov").category is MediaCategory.PUBLIC_AUTHORITY
...
def test_catalog_allows_reviewed_public_regulatory_publishers() -> None:
    expected = {
        "ftc.gov": ("Federal Trade Commission", ("ftc.gov", "www.ftc.gov")),
        "gov.uk": ("UK Government", ("gov.uk", "www.gov.uk")),
    ...
    for publisher_key, (display_name, allowed_hosts) in expected.items():
        ...
        assert profile is not None
        assert profile.display_name == display_name
        assert profile.category is MediaCategory.PUBLIC_AUTHORITY
        assert profile.article_access is ArticleAccess.ALLOWED_PUBLIC
        assert profile.allowed_hosts == allowed_hosts

    assert "news.ftc.gov" not in publisher_profile("ftc.gov").allowed_hosts
    assert "service.gov.uk" not in publisher_profile("gov.uk").allowed_hosts
...
def test_catalog_access_decisions_match_the_compliance_review() -> None:
    ...
    assert {
        profile.publisher_key: profile.article_access for profile in publisher_profiles()
    } == expected
