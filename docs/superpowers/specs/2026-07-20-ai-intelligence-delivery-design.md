...
- 即时预警：符合当前策略档位阈值的中、高风险自动推送；低风险只进入日报。
- 风险策略：支持“保守 / 默认 / 激进”三档；默认档沿用可信度 75 的自动预警规则，档位只改变推送资格与建议动作强度，不改变事实校验、证据评分或合规过滤。
...
### 5.5 `group_intelligence_preferences`

- `group_id`，主键；
- `risk_profile`：`conservative | default | aggressive`；
- `updated_at`。

该表只保存群级策略偏好，不复制群绑定码、用户标识或凭据。没有记录时使用 `INTELLIGENCE_RISK_PROFILE` 的安全默认值。档位变更与读取通过独立 repository 接口完成，不能修改历史分析、日报或 Outbox。
...
- 中风险或高风险的自动预警资格由当前风险策略决定；
- 默认策略下可信度 `>= 75`：允许自动预警；
- 默认策略下可信度 `60–74`：进入日报“待核实”，不即时推送；
- 可信度 `< 60`：保留分析记录，不进入预警；
- 低风险：不即时推送，可参与日报排序。

### 7.3 风险策略档位

风险策略是预警分发策略，不是事实真伪开关。三档共享同一套严格 Schema、原文锚定、可信元数据范围、确定性证据评分和合规来源过滤；任何档位都不能降低这些底线。

| 档位 | 即时推送条件 | 60–74 分处理 | 建议动作边界 |
|---|---|---|---|
| `conservative`（保守） | 仅高风险且可信度 `>= 85` | 只进入日报“待核实” | 以人工核实为先；自动建议只包含验证和准备动作 |
| `default`（默认） | 中风险或高风险且可信度 `>= 75` | 只进入日报“待核实” | 给出正常优先级、负责人类型和建议期限 |
| `aggressive`（激进） | 中风险或高风险且可信度 `>= 60` | 即时发送“早期信号·待核实”，并继续进入日报 | 只允许可逆的提前准备动作；不得把早期信号表述为已确认事实 |

额外约束：

- 高风险红卡只用于可信度达到 `75` 的已验证预警；激进档的 `60–74` 分事件统一使用橙色“早期信号·待核实”卡片；
- 保守档不即时推送中风险，即使可信度达到 85；该事件仍进入日报；
- 规则与模型严重冲突的结果在任何档位下都只能进入“待核实”，不得即时推送；
- 低风险和可信度 `< 60` 的处理不随档位变化；
- 档位切换不触发 24 小时内已发送事件的重复推送，也不修改历史日报、分析结果或 Outbox 载荷；
- 切换到激进档后，尚未发送且仍在候选时间窗内的 `60–74` 分事件可以在下一次预警扫描中成为早期信号。
...
- 高风险：单独红色卡片；
- 中风险：同一分析轮次合并为一张橙色卡片；
- 激进档可信度 `60–74` 的中、高风险事件：合并为橙色“早期信号·待核实”卡片，不得使用红色主题；
- 低风险：只进入日报；
- 每条内容显示当前策略档位、风险等级、证据可信度、验证状态、事件摘要、影响范围、判断依据、建议动作、负责人类型、建议期限、原文和不确定性；
...
`策略` 返回当前档位及对应推送阈值；带档位参数的命令仅在当前有效绑定群中生效，并持久化该群偏好。第一版无法可靠取得群管理员身份，因此不宣称具备管理员鉴权；回复会明确显示旧档位和新档位，后续可在飞书权限条件具备时增加管理员限制。
...
- 保守、默认、激进三档分别按“高风险且 `>=85`”“中高风险且 `>=75`”“中高风险且 `>=60`”决定即时推送资格；
- 激进档 `60–74` 分只能发送橙色“早期信号·待核实”，三档均不得降低事实依据、证据评分或合规过滤；
